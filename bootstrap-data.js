...
function labelForDate(iso){
  ...
  var y=Number(iso.slice(0,4)),t=Date.parse(iso+'T00:00:00Z');
  if(!Number.isFinite(t))return '';
  var d=Math.floor((t-Date.UTC(y,0,1))/86400000)+1;
  return iso.slice(0,4)+'-'+String(d).padStart(3,'0');
}
function validRow(row){
  if(!row||!/^2026-\d{3}$/.test(String(row.label||''))||!/^\d{4}-\d{2}-\d{2}$/.test(String(row.date||'')))return null;
  var label=String(row.label),date=String(row.date);
  if(labelForDate(date)!==label)return null;
  ...
  return {label:label,date:date,nums:nums,zodiac:nums.map(function(n){return ZODIAC[n]})};
}
...
function merge(base){
  ...
  FORMAL.forEach(function(r){byLabel.set(r.label,r)});
