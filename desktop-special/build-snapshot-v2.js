...
function hr(oldText,newText,label){if(!html.includes(oldText))throw new Error('Desktop patch target not found: '+label);html=html.replace(oldText,newText)}
...
const bandStartToken='}else if(indicator==="band"){';
const bandStartIndex=html.indexOf(bandStartToken);
const lowerBaseIndex=html.indexOf('var lowerBase=',bandStartIndex+bandStartToken.length);
const bandEndIndex=lowerBaseIndex<0?-1:html.lastIndexOf('}else{',lowerBaseIndex);
if(bandStartIndex<0||bandEndIndex<0||bandEndIndex<=bandStartIndex)throw new Error('Lower BOLL visual patch anchors not found');
...
['custom-profiles.js?v=profiles-v1','id="smp-custom-groups"','data-smp-mode="custom"','data-smp-period="5"','data-smp-chart-zoom="in"','macau-special-chart-zoom','longWindow=false','series.length*barStep+88','BOLL(20,2)','UPPER:','MID:','LOWER:'].forEach(token=>{if(!verifyHtml.includes(token))throw new Error('Desktop profile/chart snapshot verification failed: '+token)});
['macau-special-custom-zodiac-profiles-v1','方案名称','已保存组合','新建组合','重命名','删除组合','保存方案','当前方案：'].forEach(token=>{if(!verifyJs.includes(token))throw new Error('Profile manager verification failed: '+token)});
console.log('Standalone special-column snapshot + named profiles + real bars + zoom + visual BOLL pane built and verified.');
